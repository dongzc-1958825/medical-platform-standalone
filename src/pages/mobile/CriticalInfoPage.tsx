...
const CriticalInfoPage: React.FC = () => {
    ...
    // 加载已保存的数据
    useEffect(() => {
        const saved = localStorage.getItem('healthCriticalInfo');
        if (saved) {
            try {
                const parsed = JSON.parse(saved);
                setFormData(parsed);
                console.log('✅ 已加载保存的关键信息');
            } catch (e) {
                console.error('读取关键信息失败:', e);
            }
        }
    }, []);
    ...
    const handleSubmit = () => {
        ...
        alert('关键信息已保存！');
        navigate('/mobile/health/overview');
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4">
            ...
            <div className="space-y-3 sticky bottom-0 bg-gray-50 pt-4 pb-6">
                ...
                <button
                    onClick={() => navigate('/mobile/health/overview')}
