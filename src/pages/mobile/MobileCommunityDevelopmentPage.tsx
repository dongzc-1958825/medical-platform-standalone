// src/pages/mobile/MobileCommunityDevelopmentPage.tsx
      ...
      <div className="space-y-2">
        ...
        {communities.length > 5 && !showAll && (
          <button
            ...
            className="text-sm text-teal-600 hover:text-teal-700 mt-2 w-full text-center py-1 border-t border-gray-100"
          >
...
const SuggestionModal: React.FC<{
  ...
  onClose: () => void;
  user: any;
}> = ({ communityId, communityName, onPublish, onClose, user }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newSuggestion = communitySuggestionService.addSuggestion({
      diseaseId: communityId,
      diseaseName: communityName,
      title: title.trim(),
...
// ========== 主页面 ==========
const MobileCommunityDevelopmentPage: React.FC = () => {
  ...
  const loadData = async () => {
    const currentStats = await communityStatsService.updateCommunityStats(id || '', communityName);
    setStats(currentStats);

    const all = communityStatsService.getAllStats();
    setAllStats(all);

    const suggestionList = communitySuggestionService.getAllSuggestions(id);
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="sticky top-0 bg-white border-b z-10">
        <div className="flex items-center p-4">
          ...
          <h1 className="flex-1 text-center font-semibold">{communityName}社区发展</h1>
          <button 
      ...
      <div className="px-4 mt-2">
        <div className="bg-white rounded-xl p-4 shadow-sm border-l-4 border-l-teal-500">
          ...
          {!nextLevel && (
            <div className="mt-3 pt-3 border-t border-gray-100 text-center text-teal-600 text-sm font-medium">
              🏆 已达成最高等级！
            </div>
          )}
        </div>
      ...
      <div className="px-4 flex gap-2 mb-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium ${
            selectedPeriod === 'day' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-600'
          }`}
        >
          日活
        </button>
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium ${
            selectedPeriod === 'week' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-600'
          }`}
        >
          周活
        </button>
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium ${
            selectedPeriod === 'month' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-600'
          }`}
        >
          月活
        </button>
                ...
                <div key={suggestion.id} className="bg-white rounded-xl p-4 shadow-sm">
                  ...
                  <div className="flex items-center gap-4 text-sm border-t pt-3">
                    <button
                      ...
                      className={`flex items-center gap-1 ${liked ? 'text-teal-600' : 'text-gray-500'}`}
                    >
                      <ThumbsUp className={`w-4 h-4 ${liked ? 'fill-teal-600' : ''}`} />
                      {suggestion.likes || 0}
                    </button>
...
export default MobileCommunityDevelopmentPage;