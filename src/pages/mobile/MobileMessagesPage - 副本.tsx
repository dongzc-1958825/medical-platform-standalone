  ...
  >
    ...
    >
      ...
      <div className="fixed bottom-20 right-4 z-40">
        <button
          onClick={() => setShowWechatImport(true)}
          className="p-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full shadow-lg hover:from-cyan-600 hover:to-blue-600 active:scale-95 transition-all"
          title="导入微信内容"
        >
          <MessageSquare className="w-6 h-6" />
          {wechatMessageCount > 0 && (
            <span className="absolute -top-1 -right-1 bg-white text-cyan-600 text-xs w-5 h-5 rounded-full flex items-center justify-center border border-cyan-200">
              {wechatMessageCount}
            </span>
          )}
        </button>
