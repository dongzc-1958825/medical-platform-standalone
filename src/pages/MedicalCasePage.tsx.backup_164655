// src/pages/MedicalCasePage.tsx（最终修复版）
import { useState } from 'react';
// 修复1：改回原路径（假设types目录存在且正确导出）
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center space-x-2">
                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-sm">
                  {comment.author.name?.charAt(0) || comment.author.charAt(0)}
                </div>
                    ...
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center space-x-2">
                        <div className="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center text-green-600 text-xs">
                          {reply.author.name?.charAt(0) || reply.author.charAt(0)}
                        </div>
                        <span className="font-medium text-gray-900 text-sm">{reply.author.name || reply.author}</span>
                        {reply.targetUser && (
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t">
              ...
              <div className="flex items-center">
                <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-gray-600">
                  {caseItem.author?.name?.charAt(0) || caseItem.author?.charAt(0) || '?'}
                </div>
                <span className="ml-2 text-sm text-gray-600">{caseItem.author?.name || caseItem.author || '匿名'}</span>
              </div>
            </div>
